...
def get_lidar_data(nusc, sample_rec, nsweeps, min_distance):
    ...
    for _ in range(nsweeps):
        ...
        # 删除距离原点在一定半径（min_distance）内太近的点。
...
def img_transform(img, post_rot, post_tran,
    ...
    """
    # adjust image
    img = img.resize(resize_dims)   # 采用默认的双线性采样缩放图像尺寸
    img = img.crop(crop)            # img.crop(a0, b0, a1, b1)：a代表横坐标，b代表纵坐标。0：左上角坐标 1：右下角坐标
    if flip:
    ...
    A = get_rot(rotate/180*np.pi)                   # 根据角度获取旋转矩阵
