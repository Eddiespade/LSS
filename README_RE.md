...
python main.py eval_model_iou mini --modelf=./weights/pre_model.pt --dataroot=./data/nuscenes
```

### Visualize Predictions
Visualize the BEV segmentation output by a model:

**首先需要下载： v1.0-mini.tgz 和 nuScenes-map-expansion-v1.3.zip**

```
python main.py viz_model_preds mini/trainval --modelf=MODEL_LOCATION --dataroot=NUSCENES_ROOT --map_folder=NUSCENES_MAP_ROOT

python main.py viz_model_preds mini --modelf=./weights/pre_model.pt --dataroot=./data/nuscenes --map_folder=./data/nuscenes/mini
```
...
python main.py lidar_check mini --dataroot=./data/nuscenes --viz_train=False
```
...
python main.py train mini --dataroot=./data/nuscenes --logdir=./runs --gpuid=0
```
